...
// ---------------------------------------------------------------------------
export const DEFAULT_RULES = new Set([
  "project-standards",
  "typescript-conventions",
  "team-context",
]);
export const DEFAULT_SKILLS = new Set(["game-rules", "setup"]);
export const DEFAULT_COMMANDS = new Set(["setup"]);
...
export async function analyzeCursorStructure(
  ...
  const commandsCount = commands.filter(
    (c) => !DEFAULT_COMMANDS.has(c.name),
  ).length;
...
async function fetchSkills(
  ...
  const dirs = entries.filter(
    (e) => e.type === "dir" && !isBlacklistedPath(`${e.path}/SKILL.md`),
  );
...
async function fetchCommands(
  ...
  const mdFiles = entries.filter(
    (e) => e.name.endsWith(".md") && !isBlacklistedPath(e.path),
  );
